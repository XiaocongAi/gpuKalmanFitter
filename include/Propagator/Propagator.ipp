...
Acts::Propagator<S, N>::propagate(const parameters_t &start,
  ...
  using ResultType = PropagatorResult<CurvilinearParameters, typename propagator_options_t::action_type::result_type>;
  ResultType result;
  ...
  if (!state.options.aborter(result, state, m_stepper) and !pathAborter(state, m_stepper)) {
    ...
    for (; result.steps < state.options.maxSteps; ++result.steps) {
      ...
      m_navigator.status(state, m_stepper);
      state.options.action(state, m_stepper, result.result);
      if (state.options.aborter(result, state, m_stepper) or pathAborter(state, m_stepper) ) {
