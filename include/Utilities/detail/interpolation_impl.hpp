...
///
/// @tparam N power of 2
template <size_t N> struct get_dimension {
  /// exponent @c d such that \f$2^d = N \f$
  static constexpr size_t value = get_dimension<(N >> 1)>::value + 1u;
};

/// @cond
template <> struct get_dimension<2u> { static constexpr size_t value = 1u; };
/// @endcond
...
struct interpolate_impl {
  ...
  run(const Point1 &pos, const Point2 &lowerLeft, const Point3 &upperRight,
    ...
    for (size_t i = 0; i < N / 2; ++i) {
      newFields.col(i) =
          (1 - f) * fields.col(2 * i) + f * fields.col(2 * i + 1);
    }
