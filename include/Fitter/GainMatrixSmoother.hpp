...
class GainMatrixSmoother {
 ...
 public:
  ...
  ACTS_DEVICE_FUNC parameters_t* operator()(
    ...
    for (int i = filteredStates.size() - 1; i >= 0; i --) {
      ...
      smoothedCov = ((BoundSymMatrix)(*ts.parameter.filtered.covariance()))
                    - ((BoundSymMatrix)(G * (*(prev_ts->parameter.predicted.covariance()))))
                          - ((BoundSymMatrix)(*prev_ts->parameter.smoothed.covariance() * G.transpose()));
