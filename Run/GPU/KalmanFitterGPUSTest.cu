...
int main(int argc, char *argv[]) {
  ...
  std::cout << "There are " << nDevices
            << " available GPUs with the following properties:" << std::endl;
  nStreams = nDevices;
  ...
  if (useGPU) {
    ...
    // Create events and stream for each device
...
#pragma omp parallel for num_threads(nDevices) proc_bind(master)
    for (int i = 0; i < nDevices; i++) {
      ...
      GPUERRCHK(cudaEventCreate(&startEvent[i]));
      GPUERRCHK(cudaEventCreate(&stopEvent[i]));
      ...
      GPUERRCHK(cudaEventRecord(startEvent[i], 0));
      ...
      GPUERRCHK(cudaEventRecord(stopEvent[i], stream[i]));
      GPUERRCHK(cudaEventSynchronize(stopEvent[i]));
      ...
      GPUERRCHK(cudaEventRecord(stopEvent[i], 0));
      GPUERRCHK(cudaEventSynchronize(stopEvent[i]));
      GPUERRCHK(cudaEventElapsedTime(&ms, startEvent[i], stopEvent[i]));
      printf("Thread %d: Time (ms) for KF memory transfer and execution on "
             "device %d : %f\n",
             omp_get_thread_num(), i, ms);
    }
